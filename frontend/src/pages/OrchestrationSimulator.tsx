...
import type { AgentState, AgentStatus, Workstation } from '../features/orchestration-sim/types'
...
const SPRITE_IDS = ['coderBlue', 'plannerGreen', 'reviewerPurple']
...
function initAgents(run: SimRun, children: SimRun[]): AgentState[] {
...
function mergeWithNewChildren(run: SimRun, children: SimRun[], existing: AgentState[]): AgentState[] {
  const existingIds = new Set(existing.map(a => a.id))
  const newChildren: AgentState[] = []

  children.forEach((c, i) => {
    if (existingIds.has(c.id)) return
    // Assign the next sprite slot after existing sub-agents
...
const MIN_DWELL_MS = 900    // minimum visible dwell at a workstation
const MAX_DWELL_MS = Infinity // always honour the actual step duration

// ── Single-event state reducer ────────────────────────────────────────────────
...
function applySingleEvent(ev: SimEvent, agents: AgentState[], dwellMs?: number): AgentState[] {
