...
| **OS Debate Room** | OutSystems-driven multi-agent debate sessions; host + optimist + critic agents; human-in-loop mode; A2A endpoint for external orchestration |
...
```bash
# Make sure your virtual environment is activated first (see step 3)
...
A `start.bat` file at the project root opens the backend and frontend in separate terminal windows and launches the browser automatically:
...
| `AGENT_MODEL` | `openai/gpt-4o` | LLM used by the A2A agent tool-calling loop |
| `A2A_SYNTHESIZE` | `true` | Set to `false` to skip synthesis LLM call (when caller handles it) |
| `OS_DEBATE_ENDPOINT` | *(harmonygroup URL)* | OutSystems StartDebate API URL |
| `OS_API_USERNAME` | — | Basic Auth username for outbound OS API calls |
| `OS_API_PASSWORD` | — | Basic Auth password for outbound OS API calls |
...
│   └── package.json
├── outsystems/
│   ├── ONE2026.oml         # OutSystems module (OS Debate Room integration)
│   └── README.md           # Setup instructions for the OutSystems side
...
The `outsystems/` folder contains `ONE2026.oml` — the OutSystems Module that drives the **OS Debate Room** feature.

### What the module does

The OutSystems side exposes a `StartDebate` REST API that the lab backend calls to orchestrate a multi-agent debate. Three AI agents (Host, Optimist, Critic) are defined in OutSystems; the Critic calls back to the lab's local A2A endpoint so the critical argument is generated by the on-premise LLM.

```
Lab backend  ──POST /OSAgent/StartDebate──►  OutSystems (ONE2026)
                                                  │
                          ◄──POST /api/os-debate/turn──  Host / Optimist push turns
                          ◄──POST /api/os-debate/a2a ──  Critic requests local LLM
```

### Setup

1. Open `outsystems/ONE2026.oml` in **OutSystems Service Studio (ODC)**
2. Point the REST consumer at your lab backend URL (ngrok tunnel or `http://localhost:8002`)
3. Add the following to `backend/.env`:

```ini
OS_DEBATE_ENDPOINT=https://<your-env>.outsystems.app/ONE2026/rest/OSAgent/StartDebate
OS_API_USERNAME=<username configured in OutSystems>
OS_API_PASSWORD=<password configured in OutSystems>
```

4. Expose the backend via ngrok so OutSystems can call back:
   - Start the tunnel from the **Connections** page in the lab UI
   - Register the lab's A2A endpoint in OutSystems as an external connection pointing to `<ngrok-url>/api/os-debate/a2a`

See [`outsystems/README.md`](outsystems/README.md) for full details.
